...
from tensorflow.keras.optimizers import RAdam
...
def load_embeddings(file_list, emb_dir):
    ...
    for idx, filename in enumerate(file_list):
        ...
        # print("NOWWWWWWWWWWWWWWWWWWWWWWWWWWWWWw"+ os.path.splitext(filename)[0])
        with gzip.open(emb_path, 'rb') as f:
            embeddings.append(np.load(f,allow_pickle=True))
...
def train_model(model, X_train, y_train, X_valid, y_valid, output_dir,
    ...
    # Fit model
    model.compile(SGD(lr=learning_rate), loss=loss, metrics=metrics)
    history = model.fit(
